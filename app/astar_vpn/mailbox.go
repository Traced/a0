...
func (m *MailBox) GetAuthToken() string {
	resp, err := utils.GetWithHeader(mailURL, mailHeaders)
	if err != nil {
...
func (m *MailBox) QueryNewMail() (mailList NewMailResponses) {
	...
	resp, _ := utils.GetWithHeader(mailApiURL+m.Address, mailHeaders)
...
func (m *MailBox) GetMailBody(mailID string) (mb MailBodyResponse) {
	resp, err := utils.GetWithHeader(mailApiURL+m.Address+"/"+mailID, mailHeaders)
	if err != nil {
